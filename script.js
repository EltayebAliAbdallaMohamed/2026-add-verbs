...
function initControls(slidesArray, presentationInfo){
  ...
  // function to measure toolbar and set CSS var --toolbar-height
  function updateToolbarHeightVar(){
    const h = toolbar ? toolbar.getBoundingClientRect().height : 72;
    document.documentElement.style.setProperty('--toolbar-height', Math.ceil(h) + 'px');
    // ensure body uses padding to keep content below toolbar
  ...
  // Update page title
  document.title = presentationInfo && presentationInfo.title ? presentationInfo.title : document.title;
  ...
  function updateThumbnailsVisibility(){
    // Let CSS control layout; keep function for backwards compatibility.
  ...
  function updateThumbnails(){
    ...
    thumbs.forEach((t, idx) => {
      if(idx === current){
        ...
        // center visible thumb on mobile, keep nearest on desktop
        if(isMobile){
  ...
  function showSlide(idx, fromHistory){
    ...
    // On narrow screens (mobile/tablet), ensure the chosen slide is scrolled into view below the toolbar
    if(window.innerWidth <= 1024){
      const rect = slidesArray[idx].getBoundingClientRect();
      const toolbarHeight = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--toolbar-height')) || (toolbar ? Math.ceil(toolbar.getBoundingClientRect().height) : 72);
      const target = window.scrollY + rect.top - toolbarHeight - 8; // small gap
  ...
  // Click / tap to advance (mobile-friendly)
  slidesContainer.addEventListener('click', (e) => {
    if(e.target.closest('button')) return;
    if(isMobile){
      // On mobile, tap right half => next, left half => prev
      ...
      // desktop click (not on a control) advances
      next();
    }
  });
